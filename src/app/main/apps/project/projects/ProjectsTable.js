...
import { getProjects, selectProjects, selectProjectsSearchText } from '../store/projectsSlice';
...
function ProjectsTable(props) {
  ...
  useEffect(() => {
    if (searchText.length !== 0) {
      setData(
        _.filter(projects, (item) => item.name.toLowerCase().includes(searchText.toLowerCase()))
      );
      setPage(0);
    } else {
      setData(projects);
